...
describe('Avatar Storage', () => {
  ...
  describe('generateAvatarFilename', () => {
    beforeEach(() => {
      jest.useFakeTimers();
      jest.setSystemTime(new Date('2024-02-21T12:00:00Z'));
    });
  ...
  describe('uploadAvatar', () => {
    ...
    it('should return public URL', async () => {
      const file = new File(['image data'], 'avatar.jpg', { type: 'image/jpeg' });
      const publicUrl =
        'https://storage.example.com/avatars/user-123/avatar.jpg';
  ...
  describe('deleteAvatar', () => {
    ...
    it('should throw error if deletion fails', async () => {
      ...
      await expect(
        deleteAvatar(mockSupabase, 'user-123/avatar.jpg')
      ).rejects.toThrow('Failed to delete avatar');
    });
  });
  ...
  describe('getAvatarPublicUrl', () => {
    it('should return public URL for avatar', () => {
      ...
      const result = getAvatarPublicUrl(
        mockSupabase,
        'user-123/avatar.jpg'
      );

      expect(result).toBe(publicUrl);
      expect(mockBucket.getPublicUrl).toHaveBeenCalledWith(
        'user-123/avatar.jpg'
      );
    });
  });
